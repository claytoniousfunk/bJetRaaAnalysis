...
// calculateRAA.C chain: stitched spectra), fine pT bins, WITHOUT unfolding
// (doUnfolding=false -- the measured spectra used directly), comparing the
// plain-RC and RC_geoCorr fake-jet subtraction methods, per coarse centrality
// class. See plotJetsPerZ_RCvsRCgeoCorr_unfold.C for the otherwise-identical
// unfolded twin.
//
...
// Reco-level (no unfolding) rises much higher at low pT than the unfolded
// version -- 0-10% reaches 2.49 at 60-65 GeV, an unsuppressed resolution/
// migration effect the unfolding step exists to correct for. Wider range than
// the unfolded twin's [0.2,1.05] for exactly that reason.
const double topMin = 0.2, topMax = 2.7;
...
void plotJetsPerZ_RCvsRCgeoCorr_noUnfold(){
  ...
  TCanvas *c = new TCanvas("cJetsPerZ_RCvsRCgeoCorr_noUnfold", "", 800, 800);
  ...
  lat.DrawLatex(0.19, 0.955, "Jets per Z, with stitching, NOT unfolded -- PbPb / pp");
  ...
  TString out = outDir + "JetsPerZ_RCvsRCgeoCorr_noUnfold.pdf";
  c->SaveAs(out);
  printf("\nSaved %s\n", out.Data());
}
