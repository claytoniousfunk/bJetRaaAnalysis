...
// calculateRAA.C chain), fine pT bins, WITHOUT unfolding (doUnfolding=false),
// comparing three fake-jet subtraction methods: plain RC, RC_geoCorr, and dPT
// (PF-PFCs). Extends plotJetsPerZ_RCvsRCgeoCorr_noUnfold.C ("for
// completeness") with the dPT method. See
// plotJetsPerZ_fakeSubMethods_unfold.C for the unfolded twin.
//
...
//
// *** dPT over-subtracts far worse without unfolding: the jets-per-Z numerator
// goes negative for 4 of 20 fine bins in 0-10% (60-80 GeV), down to -2.00 at
// 60-65 GeV -- roughly 6x deeper than the unfolded version's -0.34. Every
// other class stays positive throughout. The top-panel range is widened
// substantially (to +-~2.5) to show the excursion rather than clip it. ***
//
// Reads r_C{1..4}_fine from three calculateRAA.C runs, identical except for
// which fake-jet estimate was subtracted:
//   histograms_stitched_RC_noUnfold.root        : fakeJetsPath = fakeJets.root
//   histograms_stitched_RCgeoCorr_noUnfold.root : fakeJetsPath = fakeJets_RCgeoCorr.root
//   histograms_stitched_dPT_noUnfold.root       : fakeJetsPath = fakeJets_dPT.root
// All three with useMinBiasOnly=false, doUnfolding=false, e.g.:
//   root -l -b -q -e '.L calculateRAA.C' \
//     -e 'useMinBiasOnly=false; doUnfolding=false; fakeJetsPath="../../rootFiles/fakeJets/fakeJets_dPT.root"; \
//         outputRootPath="./rootFiles/JetsPerZ/histograms_stitched_dPT_noUnfold.root";' \
//     -e 'calculateRAA()'
//
// Usage: root -l -b -q 'plotJetsPerZ_fakeSubMethods_noUnfold.C'
// Run from: src/newFractionCalculation/

const char *f_RC_path =
  "rootFiles/JetsPerZ/histograms_stitched_RC_noUnfold.root";
const char *f_RCgeoCorr_path =
  "rootFiles/JetsPerZ/histograms_stitched_RCgeoCorr_noUnfold.root";
const char *f_dPT_path =
  "rootFiles/JetsPerZ/histograms_stitched_dPT_noUnfold.root";
...
const double topMin = -2.3, topMax = 2.8;
const double botMin = -3.5, botMax = 2.5;

void plotJetsPerZ_fakeSubMethods_noUnfold(){
  ...
  TCanvas *c = new TCanvas("cJetsPerZ_fakeSubMethods_noUnfold", "", 800, 800);
  ...
  lat.DrawLatex(0.17, 0.955, "Jets per Z, with stitching, NOT unfolded -- PbPb / pp");
  ...
  TString out = outDir + "JetsPerZ_fakeSubMethods_noUnfold.pdf";
  c->SaveAs(out);
  printf("\nSaved %s\n", out.Data());
}
