// Build rootFiles/fakeJets/fakeJets_RCgeoCorr.root -- the geometric-correction
// twin of makeFakeJetFile.C's fakeJets.root.
//
// Fake-jet estimate = FastJet, JEC-corrected pT, with the per-jet background
// subtracted using the geometrically-corrected random cone (each candidate
// weighted by cos(dR) to the cone axis before summing, instead of the plain
// unweighted cone sum) instead of the plain random cone:
//   FastJet  : h_fastJetPt_PF_JEC_bkgSub_RC_geoCorr_C{i} / N_events   (one clustering per event)
//   RandomCone: h_pseudoJetPt_C{i}                       / (N_events*N_pool)  (N_pool cones per event, for reference only)
//
// Only difference from makeFakeJetFile.C: fjHistBase points at the RC_geoCorr
// background-subtraction variant instead of the plain RC one. Everything else --
// slice summing, per-event normalization, the pT_min floor, zeroing negative
// bins -- is identical; see makeFakeJetFile.C for the reasoning.
//
// The random-cone baseline subtraction is currently disabled (see below); the
// per-jet geoCorr background subtraction is already folded into
...
// Which FastJet spectrum to use as the fake estimate -- RC_geoCorr here, deliberately.
const char *fjHistBase = "h_fastJetPt_PF_JEC_bkgSub_RC_geoCorr";
...
void makeFakeJetFile_RCgeoCorr(){
