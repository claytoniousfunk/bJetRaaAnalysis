...
const char *inFile =
  "/home/clayton/Analysis/code/bJetRaaAnalysis/rootFiles/scanningOuput/PbPb/PbPb_MinBias_Part1_mu12_pTmu-15to999_tight_jetTrkMaxFilter_WDecayFilter_sameEventPFClustering_pseudoJetCandPtMin-0.0_RCGeoCorrMapBkgSub_2026-8-6_ultraFineCentBins.root";
...
void plotRandConeEtaPhi()
{
  ...
  for(int ci = 0; ci < NCent; ci++){
    ...
    //f->GetObject(Form("h_randConeEtaPhi_geoCorr_C%d", ci), hMap[ci]);
    //f->GetObject(Form("h_dPTEtaPhi_PF_PFCs_C%d", ci), hMap[ci]);
    //f->GetObject(Form("h_dPTEtaPhi_PF_PFCs_dPTAbove0_C%d", ci), hMap[ci]);
    f->GetObject(Form("h_dPTEtaPhi_PF_PFCs_PFCsPTAbove60_C%d", ci), hMap[ci]);
