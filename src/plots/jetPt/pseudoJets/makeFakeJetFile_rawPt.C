// Build rootFiles/fakeJets/fakeJets_rawPt.root -- the raw-axis twin of
// makeFakeJetFile.C's fakeJets.root.
//
...
//
// Only difference from makeFakeJetFile.C: fjHistBase points at the RAW
// (non-JEC) FastJet spectrum, so the output's x axis is raw pT. That makes it
// the correct fake-jet estimate to subtract from a genuine raw-pT reco-jet
// spectrum (h_inclRawJetPt in PbPb_scan.C), where makeFakeJetFile.C's own
// output (JEC-axis) would be an axis mismatch. See that file for the reasoning
// on why the JEC variant is normally the right choice for h_inclRecoJetPt --
// this file exists specifically for the case where it is not.
//
...
// Which FastJet spectrum to use as the fake estimate -- raw here, deliberately.
const char *fjHistBase = "h_fastJetPt_PF_bkgSub_RC";
...
const char *outPath =
  "/home/clayton/Analysis/code/bJetRaaAnalysis/rootFiles/fakeJets/fakeJets_rawPt.root";
