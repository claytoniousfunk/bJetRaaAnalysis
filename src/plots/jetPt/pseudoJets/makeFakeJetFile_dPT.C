// Build rootFiles/fakeJets/fakeJets_dPT.root -- the dPT-subtraction twin of
// makeFakeJetFile.C's fakeJets.root.
//
// Fake-jet estimate = FastJet, JEC-corrected pT, with the per-jet background
// subtracted using the Delta-pT (PF-PFCs) method instead of the random-cone
// method:
//   FastJet  : h_fastJetPt_PF_JEC_bkgSub_dPT_C{i} / N_events   (one clustering per event)
//   RandomCone: h_pseudoJetPt_C{i}                / (N_events*N_pool)  (N_pool cones per event, for reference only)
//
// Only difference from makeFakeJetFile.C: fjHistBase points at the dPT
// background-subtraction variant instead of the RC one. Everything else --
...
// per-jet dPT background subtraction is already folded into
...
// Which FastJet spectrum to use as the fake estimate -- dPT here, deliberately.
const char *fjHistBase = "h_fastJetPt_PF_JEC_bkgSub_dPT";
