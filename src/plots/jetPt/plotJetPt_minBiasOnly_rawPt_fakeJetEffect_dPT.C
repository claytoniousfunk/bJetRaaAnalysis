...
// random-cone one: rootFiles/fakeJets/fakeJets_dPT_rawPt.root, built by
// makeFakeJetFile_dPT_rawPt.C from h_fastJetPt_PF_bkgSub_dPT. PbPb only, no pp
// side, same as the RC version. Not the file calculateRAA.C actually
// subtracts -- exists to compare background-subtraction methods.
//
...
// estimate here is h_fakeJets_C{1..4} from fakeJets_dPT_rawPt.root (built by
// makeFakeJetFile_dPT_rawPt.C, which points at h_fastJetPt_PF_bkgSub_dPT -- the
// RAW, non-JEC, dPT-subtracted FastJet spectrum), so subtracting it from a genuine raw-pT reco-jet
...
//
// Usage: root -l -b -q 'plotJetPt_minBiasOnly_rawPt_fakeJetEffect_dPT.C'
...
const char *outName = "jetPt_minBiasOnly_rawPt_fakeJetEffect_dPT.pdf";
...
void plotJetPt_minBiasOnly_rawPt_fakeJetEffect_dPT()
{
  gStyle->SetOptStat(0);
  ...
  TCanvas *c = new TCanvas("cRawPtFakeJetEffectDPT", "", 800, 800);
  ...
  lat.DrawLatex(0.19, 0.91, "Raw p_{T} axis, dPT background subtraction. Open = no sub, filled = with");
