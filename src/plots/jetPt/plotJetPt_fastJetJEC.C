...
//   - inclusive CMS reco jets
//   - FastJet anti-kT R=0.4 (CS, no JEC)
//   - FastJet anti-kT R=0.4 (CS, JEC applied)
// from HYDJET pfCandAnalyzer output.
// Normalizes all spectra over the visible pT range [pTlo, pThi].
// Usage: root -l -q 'plotJetPt_fastJetJEC.C'
...
void plotOneBin(TFile *f, int ci, const char *centLbl, double lm, double rm){
  ...
  TH1D *hReco = (TH1D*) h2->ProjectionX(Form("hReco_C%d", ci));
  ...
  TH1D *hFJ_JEC = nullptr;
  f->GetObject(Form("h_fastJetPt_JEC_C%d", ci), hFJ_JEC);
  if(!hFJ_JEC){ printf("ERROR: missing h_fastJetPt_JEC_C%d\n", ci); return; }
  hFJ_JEC = (TH1D*) hFJ_JEC->Clone(Form("hFJ_JEC_C%d", ci));
  hFJ_JEC->SetDirectory(0);
  ...
  hFJ_JEC->Scale(1. / hFJ_JEC->Integral(blo, bhi));

  TH1D *hRatioFJ     = (TH1D*) hFJ->Clone(Form("hRatioFJ_C%d", ci));
  hRatioFJ->Divide(hReco);
  TH1D *hRatioFJ_JEC = (TH1D*) hFJ_JEC->Clone(Form("hRatioFJ_JEC_C%d", ci));
  hRatioFJ_JEC->Divide(hReco);
  ...
  hReco->SetLineColor(kBlue+1); hReco->SetLineWidth(2); hReco->SetLineStyle(1);
  ...
  hFJ_JEC->SetLineColor(kGreen+2); hFJ_JEC->SetLineWidth(2); hFJ_JEC->SetLineStyle(3);
  hFJ_JEC->Draw("same");

  TLegend *leg = new TLegend(0.45, 0.60, 0.92, 0.88);
  leg->SetBorderSize(0); leg->SetTextSize(0.052);
  leg->AddEntry(hReco,   "inclusive reco jets", "l");
  leg->AddEntry(hFJ,     "FastJet anti-k_{T} R = 0.4 (CS)", "l");
  leg->AddEntry(hFJ_JEC, "FastJet anti-k_{T} R = 0.4 (CS + JEC)", "l");
  leg->Draw();

  TLatex *lat = new TLatex(); lat->SetNDC(); lat->SetTextSize(0.052);
  ...
  hRatioFJ->SetLineColor(kRed+1); hRatioFJ->SetLineWidth(2); hRatioFJ->SetLineStyle(2);
  hRatioFJ->GetXaxis()->SetRangeUser(pTlo, pThi);
  hRatioFJ->GetXaxis()->SetTitle("p_{T}^{jet} [GeV]");
  hRatioFJ->GetYaxis()->SetTitle("FastJet / Reco");
  hRatioFJ->GetYaxis()->SetRangeUser(0, 2.5);
  hRatioFJ->GetXaxis()->SetTitleSize(0.07); hRatioFJ->GetXaxis()->SetLabelSize(0.065);
  hRatioFJ->GetYaxis()->SetTitleSize(0.07); hRatioFJ->GetYaxis()->SetLabelSize(0.065);
  hRatioFJ->GetYaxis()->SetTitleOffset(1.1);
  hRatioFJ->GetYaxis()->SetNdivisions(505);
  hRatioFJ->SetStats(0); hRatioFJ->SetTitle("");
  hRatioFJ->Draw();

  hRatioFJ_JEC->SetLineColor(kGreen+2); hRatioFJ_JEC->SetLineWidth(2); hRatioFJ_JEC->SetLineStyle(3);
  hRatioFJ_JEC->Draw("same");

  TLine *l = new TLine(pTlo, 1., pThi, 1.);
  l->SetLineStyle(2); l->SetLineColor(kGray+1); l->SetLineWidth(2); l->Draw();

  TString outFile = TString(outDir) + Form("fastJetJEC_vs_reco_shape_C%d.pdf", ci);
  c->SaveAs(outFile);
  printf("Saved %s\n", outFile.Data());
