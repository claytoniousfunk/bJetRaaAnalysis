// Double ratio: does the fake-jet subtraction's (with/without) effect look
// different depending on whether jets are indexed by JEC-corrected pT or by
// raw pT?
//
//   DR(pT) = [with/without]_recoPt(pT)  /  [with/without]_rawPt(pT)
//
// Each factor is built exactly as in the two single-axis macros this combines:
//   plotJetPt_minBiasOnly_PbPbOverPP_fakeJetEffect.C   (recoPt / JEC axis)
//   plotJetPt_minBiasOnly_rawPt_fakeJetEffect.C         (rawPt axis)
...
//
// Usage: root -l -b -q 'plotJetPt_minBiasOnly_fakeJetEffect_recoPtOverRawPt.C'
...
const char *outName = "jetPt_minBiasOnly_fakeJetEffect_recoPtOverRawPt.pdf";
...
void plotJetPt_minBiasOnly_fakeJetEffect_recoPtOverRawPt()
{
  gStyle->SetOptStat(0);
  ...
  TCanvas *c = new TCanvas("cDR", "", 750, 600);
  ...
  lat.DrawLatex(0.17, 0.90, "Double ratio of the fake-jet subtraction's (with/without) effect");
