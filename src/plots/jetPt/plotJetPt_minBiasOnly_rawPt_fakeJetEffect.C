// Effect of a raw-pT-consistent fake-jet subtraction on the PbPb MinBias
// raw-pT jet spectrum, per coarse centrality class. PbPb only -- no pp side.
//
...
// estimate here is h_fakeJets_C{1..4} from fakeJets_rawPt.root (built by
// makeFakeJetFile_rawPt.C, which points at h_fastJetPt_PF_bkgSub_RC -- the RAW,
// non-JEC FastJet spectrum), so subtracting it from a genuine raw-pT reco-jet
...
//
// Usage: root -l -b -q 'plotJetPt_minBiasOnly_rawPt_fakeJetEffect.C'
...
const char *outName = "jetPt_minBiasOnly_rawPt_fakeJetEffect.pdf";
...
void plotJetPt_minBiasOnly_rawPt_fakeJetEffect()
{
  gStyle->SetOptStat(0);
  ...
  TCanvas *c = new TCanvas("cRawPtFakeJetEffect", "", 800, 800);
  ...
  lat.DrawLatex(0.19, 0.91, "Raw p_{T} axis throughout. Open = no subtraction, filled = with (raw-axis fakeJets)");
