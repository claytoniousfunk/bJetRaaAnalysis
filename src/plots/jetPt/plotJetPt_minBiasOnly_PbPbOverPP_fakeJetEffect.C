// Effect of the current mixed-event fake-jet subtraction (rootFiles/fakeJets/
// fakeJets.root, built by makeFakeJetFile.C -- the same file calculateRAA.C
// subtracts) on the PbPb/pp MinBias shape comparison, per coarse centrality
// class.
//
...
// h_fakeJets_C{1..4} in fakeJets.root is a PER-EVENT fake-jet rate (fakes per
...
// same convention as makeFakeJetFile.C) before the bin-by-bin subtraction. This
...
// in fakeJets.root. Bins that go negative after subtraction are zeroed, same
...
// from.
//
// Usage: root -l -b -q 'plotJetPt_minBiasOnly_PbPbOverPP_fakeJetEffect.C'
...
const char *outName = "jetPt_minBiasOnly_PbPbOverPP_fakeJetEffect.pdf";
...
// fakeJets.root indexes the same four classes as h_fakeJets_C1..C4.
const int   NClass = 4;
...
void plotJetPt_minBiasOnly_PbPbOverPP_fakeJetEffect()
{
  gStyle->SetOptStat(0);
  ...
  TCanvas *c = new TCanvas("cFakeJetEffect", "", 800, 800);
  ...
  lat.DrawLatex(0.19, 0.91, "Open = no fake-jet subtraction, filled = with (current fakeJets.root)");
