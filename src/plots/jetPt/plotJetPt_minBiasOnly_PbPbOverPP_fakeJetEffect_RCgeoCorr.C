...
// the plain random cone: rootFiles/fakeJets/fakeJets_RCgeoCorr.root, built by
// makeFakeJetFile_RCgeoCorr.C from h_fastJetPt_PF_JEC_bkgSub_RC_geoCorr. NOT the
// file calculateRAA.C actually subtracts -- that is still the plain-RC
// fakeJets.root. This exists to compare background-subtraction methods.
//
// RC_geoCorr predicts more fakes than plain RC but far fewer than dPT: 4.77
// fakes/event vs 3.46 (RC) and 8.73 (dPT) in 0-10%, all from the same
// mixed-event file.
//
...
// h_fakeJets_C{1..4} in fakeJets_RCgeoCorr.root is a PER-EVENT fake-jet rate (fakes per
...
// same convention as makeFakeJetFile_RCgeoCorr.C) before the bin-by-bin subtraction. This
...
// in fakeJets_RCgeoCorr.root. Bins that go negative after subtraction are zeroed, same
...
// from.
//
// Usage: root -l -b -q 'plotJetPt_minBiasOnly_PbPbOverPP_fakeJetEffect_RCgeoCorr.C'
...
const char *outName = "jetPt_minBiasOnly_PbPbOverPP_fakeJetEffect_RCgeoCorr.pdf";
...
// fakeJets_RCgeoCorr.root indexes the same four classes as h_fakeJets_C1..C4.
const int   NClass = 4;
...
void plotJetPt_minBiasOnly_PbPbOverPP_fakeJetEffect_RCgeoCorr()
{
  gStyle->SetOptStat(0);
  ...
  TCanvas *c = new TCanvas("cFakeJetEffectRCgeoCorr", "", 800, 800);
  ...
  lat.DrawLatex(0.19, 0.91, "Open = no fake-jet subtraction, filled = with (RC_geoCorr fakeJets_RCgeoCorr.root)");
