// Wrapper to compile PbPb_pfCandAnalyzer_rollingPool.C with ACLiC + FastJet support.
// Usage: root -l -q 'run_pfCandAnalyzer_rollingPool.C(1)'
// Requires the LCG view (or CMSSW) to be sourced so that fastjet-config is in PATH.

void run_pfCandAnalyzer_rollingPool(int group = 1){
  ...
  gROOT->LoadMacro("PbPb_pfCandAnalyzer_rollingPool.C+");
  gROOT->ProcessLine(Form("PbPb_pfCandAnalyzer_rollingPool(%d)", group));
}
