...
inline void writeProvenance(TFile *wf)
{
  ...
  snprintf(b, sizeof(b), "bkgMapFile                  : %s\n", bkgMapFileUsed.c_str());             s += b;
  snprintf(b, sizeof(b), "caloConstituentsOnly        : %d (includeCharged %d)\n",
           (int)caloConstituentsOnly, (int)caloConstituentsIncludeCharged);                             s += b;
